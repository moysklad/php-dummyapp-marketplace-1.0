...
* Пример авторизации запросов из iframe/виджета на backend через короткоживущий подписанный `contextToken` (JWT)
...
То же относится к demo-механизму `contextToken` для авторизации запросов на backend.

Для production-решения как минимум потребуется:
* полноценная серверная авторизация и проверка прав на каждый запрос к бэкенду;
* защита от CSRF и replay-атак, аудит `jti` и сроков жизни токенов;
* стратегия хранения и очистки session/context данных (TTL, инвалидирование, ротация);
* централизованное безопасное хранение состояния (не только файловая/локальная сессия).
...
* Отображение данных контекста, сохраненных в PHP-сессии (источник контекста, session id, история)
* Авторизованный запрос к backend (`get-object.php`) с `Authorization: Bearer <contextToken>`
