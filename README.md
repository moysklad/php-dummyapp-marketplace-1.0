...
PHP-сессии с `contextKey` используются отдельно только для demo-авторизации backend-вызовов из iframe/виджетов.
Сессионный кэш контекстов ограничен 10 `contextKey` на сессию, срок жизни — 2 часа.
Cookie сессии настроены в HTTPS-only режиме: `SameSite=None`, `Secure=true`.
Блоки с PHP-сессией и `contextKey` реализованы строго как демонстрация и не являются production-реализацией.
